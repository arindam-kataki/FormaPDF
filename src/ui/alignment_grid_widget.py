...
class AlignmentGridWidget(QWidget):
    """3x3 grid widget for selecting text alignment"""

    alignmentChanged = pyqtSignal(str)  # Emits combined alignment like "top-left"

    def __init__(self):
        super().__init__()
        self.current_alignment = "middle-center"  # Default center alignment
        self.init_ui()

    def init_ui(self):
        """Initialize the 3x3 alignment grid"""
        layout = QVBoxLayout()
        layout.setSpacing(2)
        ...
        # Button group to handle exclusive selection
        self.button_group = QButtonGroup()
        self.button_group.setExclusive(True)

        # Define the 3x3 grid positions and their corresponding alignments
        self.alignment_map = {
            (0, 0): ("top-left", "⬉"),
            (0, 1): ("top-center", "⬆"),
            (0, 2): ("top-right", "⬈"),
    ...
    def set_alignment_from_components(self, horizontal: str, vertical: str):
        """Set alignment from separate horizontal and vertical components"""
        combined = f"{vertical}-{horizontal}"
        if combined in self.buttons:
            self.set_alignment(combined)
...
class AlignmentGridWidget(QWidget):
    ...
    def init_ui(self):
        ...
        grid_widget.setLayout(grid_layout)
        layout.addWidget(grid_widget, 0, Qt.AlignmentFlag.AlignCenter)
